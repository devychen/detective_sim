# extract_holmes.py
# Aim: extract holmes quotes from original books
...
for filename in os.listdir(input_dir):
    if filename.lower().endswith(".txt"):
        ...
        for match in matches:
            ...
            if (any(name in context_before for name in holmes_keywords) and
                any(verb in context_before for verb in speak_verbs)):
                candidate = quote
            elif (any(name in context_after for name in holmes_keywords) and
            ...
            if 10 <= len(tokens) <= 100:
                holmes_lines.append(candidate)

# 5. 保存到 CSV
os.makedirs("lines", exist_ok=True)
with open("lines/holmes_lines.csv", "w", newline="", encoding="utf-8") as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["number", "quote"])
    for i, line in enumerate(holmes_lines, 1):
        writer.writerow([i, line])

print(f"Extraction completed, found {len(holmes_lines)} lines in total")
