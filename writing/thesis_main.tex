...
\begin{itemize}[nosep]
    \item Use only known facts and dialogue history—never invent clues.
    \item Provide detailed reasoning and evidence to aid deduction.
    \item Only one murderer exists. Unnamed people are irrelevant (neither suspects nor accomplices).
    \item Share your full exclusive clues to others for collaboration.
\end{itemize}

However, in trial runs (to test whether the overall design works), I noticed that some of the collaboration rules were occasionally violated. To ensure these rules are followed, I added an additional emphasis on key instructions in the function that builds the system prompt: after assembling the three modules, the same function re-states the important rules to guarantee that the resulting data are suitable for analysis. The added instructions are as follows:
