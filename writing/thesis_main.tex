...
\subsubsection{Case-solving rate} \label{sec:background_taskacc}
...
GPT-4 was then instructed to act as a professional literary analysis assistant" and generate the final persona prompts (namely the Role-Play Guideline) based on the character description generated. The system message was:
\begin{minted}{text}
...
The evaluation has two primary axes. First one is also the core concept in this study.

% The evaluation scheme is designed to rigorously assess the trade-off between case-solving rate (correctly solving the mystery) and character consistency (remaining in character) within a multi-agent detective simulation. 
...
% \subsection{Evaluation Overview} \label{sec:eval_overciew}

Each simulation run is evaluated across two primary axes, reflecting the agent's problem solving basic reasoning ability, and its role-playing quality. The two axes are :
