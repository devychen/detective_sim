...
Inspired by \citet{wu2024jubensha}, the full system prompt consists of six components with the following headers: \textbf{Background} (fixed), \textbf{Collaboration Rules} (fixed), \textbf{Role Play Guidelines} (fixed), \textbf{Protective Guidelines} (fixed), \textbf{Allocated Case Information} (fixed), and \textbf{Conversation So Far}, which is dynamically appended as the interaction grows.

After each turn, the conversation history is updated. As mentioned in Section \ref{sec:methods_model}, explicit delimiters and headers are used to ensure clear logical partitioning. In the Python script, this appears as:

\begin{itemize}[itemsep=0pt, parsep=0pt, topsep=7pt]
    \item[] \texttt{=== Background ===}
    \item[] ...
    \item[] \texttt{=== Collaboration Rules ===}
    \item[] ...
    \item[] \texttt{=== Role-Play Guidelines ===}
    \item[] ...
    \item[] \texttt{=== Protective Guidelines ===}
    \item[] ...
    \item[] \texttt{=== Allocated Case Information ===}
    \item[] ...
    \item[] \texttt{=== Conversation So Far ===}
    \item[] ...
\end{itemize}
