...
\subsection{Overview of Findings: Addressing Research Questions}

Based on the experiement results, now research questions and hypotheses could be evaluted. This section provides a summary of the findings and directly addresses the research questions and hypotheses of this study. The results show that character consistency is not a single, uniform state. Instead, it depends on which linguistic level is being measured
...
The results partially support the first hypothesis. LLM agents maintained character consistency at lexical and syntactic levels. However, this consistency was not perfect. As the collaborative dialogues progressed, agents showed systematic inconsistency. Their sentence structures became more complex and less like the original literary sources. Therefore, character consistency is partially preserved but tends to erode during long interactions.
...
The results support the second hypothesis. A single metric cannot fully capture the nuances of character consistency and multiple linguistic levels provides a better view. Lexical and syntactic features acted as strong character anchors. These structural patterns remained detectable even under the pressure of collaboration. In contrast, embedding-based clustering showed representational convergence. Combining these levels provides a more complete view of how agents behave in multi-agent settings
...
The results support the third hypothesis. Syntactic depth metrics showed the strongest correlation with classifier confidence scores. Specifically, raw depth deviation was the most reliable indicator of character consistency. This confirms that structural patterns are better predictors of character fidelity than lexical or discourse features.


\subsection{a multi-level interpretation of character consistency}

The findings suggest that charcater consistency in LLM agents is not uniform. Instead, it is a layered construct where different linguistic levels contribute uneveny to identity recognition, and some features functioning as stronger signals than others.

At lexical and ysntacti level. 
